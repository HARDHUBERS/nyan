...
function DropdownElement:Create()
    ...
    self.Arrow = CreateInstance("ImageLabel", {
        ...
        ImageTransparency = 0, -- Явно указываем прозрачность
...
-- Добавляем элемент Label (метка) к библиотеке
local LabelElement = {}
...
function LabelElement:Create()
    ...
    self.LabelFrame = CreateInstance("Frame", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundColor3 = self.Theme.ElementBackground,
        BackgroundTransparency = 0.9, -- Метка почти прозрачная
        Parent = self.Container
    })
    ...
    self.TextLabel = CreateInstance("TextLabel", {
        ...
        Position = UDim2.new(0, 10, 0, self.Description ~= "" and 8 or (30 - self.TextSize) / 2),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamSemibold,
        Text = self.Text,
        TextColor3 = self.Theme.TextColor,
...
function LabelElement:UpdateSize()
    -- Рассчитываем высоту контейнера по содержимому
    local height = 30 -- Минимальная высота
    
    if self.Description and self.Description ~= "" then
        -- Рассчитываем высоту текста описания
        local textService = game:GetService("TextService")
        local textSize = self.DescriptionLabel.TextSize
        local frameWidth = self.LabelFrame.AbsoluteSize.X - 20 -- Учитываем отступы
        ...
        height = self.TextSize + 10 + textBounds.Y + 10 -- Заголовок + отступ + описание + отступ
    end
...
function WindowCreator:Create()
    ...
    -- Создаем систему вкладок (используем встроенный TabSystem)
    -- В автономной версии не можем использовать require
    local TabSystemInstance = {}
    TabSystemInstance.__index = TabSystemInstance
    
    -- Создаем экземпляр TabSystem
    self.TabSystem = setmetatable({}, TabSystemInstance)
    self.TabSystem.Window = self
    self.TabSystem.TabContainer = self.TabContainer
    self.TabSystem.ContentContainer = self.ContentContainer
    self.TabSystem.Theme = self.Theme
    self.TabSystem.Tabs = {}
    self.TabSystem.TabCount = 0
    self.TabSystem.SelectedTab = nil
    
    -- Создаем методы для TabSystem
    -- Добавление вкладки
    self.TabSystem.CreateTab = function(ts, tabInfo)
        local tabIndex = ts.TabCount + 1
        ts.TabCount = tabIndex
        
        -- Создаем кнопку вкладки
        local tabButton = CreateInstance("TextButton", {
            Size = UDim2.new(1, -10, 0, 36),
            BackgroundColor3 = ts.Theme.ElementBackground,
            BackgroundTransparency = 0.8,
            Text = "",
            AutoButtonColor = false,
            LayoutOrder = tabIndex,
            Parent = ts.TabContainer
        })
        ...
        -- Текст вкладки
        local tabText = tabInfo.Title or "Tab " .. tabIndex
        local tabIcon = tabInfo.Icon
        
        local titleLabel = CreateInstance("TextLabel", {
            Size = UDim2.new(1, tabIcon and -40 or -20, 1, 0),
            Position = UDim2.new(0, tabIcon and 40 or 10, 0, 0),
            BackgroundTransparency = 1,
            Font = Enum.Font.GothamSemibold,
            Text = tabText,
            TextColor3 = ts.Theme.TextColor,
            TextTransparency = 0.4, -- Неактивная вкладка слегка прозрачная
            TextSize = 14,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = tabButton
        })
        ...
        if tabIcon then
            iconImage = CreateInstance("ImageLabel", {
                ...
                BackgroundTransparency = 1,
                Image = tabIcon,
                ImageColor3 = ts.Theme.TextColor,
        ...
        local contentFrame = CreateInstance("ScrollingFrame", {
            ...
            ScrollBarImageColor3 = ts.Theme.AccentColor,
            ScrollBarImageTransparency = 0.5,
            CanvasSize = UDim2.fromScale(0, 0),
            AutomaticCanvasSize = Enum.AutomaticSize.Y,
            Visible = false,
            Parent = ts.ContentContainer
        })
        ...
        tabButton.MouseButton1Click:Connect(function()
            ts:SelectTab(tabIndex)
        end)
        
        -- Сохраняем вкладку
        ts.Tabs[tabIndex] = tab
        ...
        local function AddElement(elementType, options)
            tab.ElementCount = tab.ElementCount + 1
            
            -- Создаем элемент указанного типа
            local element = NYAN.Elements[elementType].new(contentFrame, options)
            ...
            -- Применяем тему
            element:ApplyTheme(ts.Theme)
            
            return element
        end
        
        -- Добавление разных типов элементов
        tabInterface.AddButton = function(options) return AddElement("Button", options) end
        tabInterface.AddToggle = function(options) return AddElement("Toggle", options) end
        tabInterface.AddSlider = function(options) return AddElement("Slider", options) end
        tabInterface.AddTextbox = function(options) return AddElement("Textbox", options) end
        tabInterface.AddDropdown = function(options) return AddElement("Dropdown", options) end
        tabInterface.AddLabel = function(options) return AddElement("Label", options) end
        
        -- Получение информации о вкладке
        tabInterface.GetInfo = function()
            return {
                Index = tab.Index,
                Title = tab.Title,
                Icon = tab.Icon,
    ...
    -- Выбор вкладки
    self.TabSystem.SelectTab = function(ts, index)
        if not ts.Tabs[index] then return end
        
        local previousTab = ts.SelectedTab
        ts.SelectedTab = index
        
        -- Скрываем предыдущую вкладку
        if previousTab and ts.Tabs[previousTab] then
            ts.Tabs[previousTab].Content.Visible = false
            
            -- Сбрасываем кнопку предыдущей вкладки
            TweenService:Create(
                ts.Tabs[previousTab].Button, 
                TAB_TWEEN, 
                {BackgroundTransparency = 0.8}
            ):Play()
            
            -- Делаем текст и иконку полупрозрачными
            for _, child in pairs(ts.Tabs[previousTab].Button:GetChildren()) do
        ...
        ts.Tabs[index].Content.Visible = true
        
        -- Обновляем кнопку выбранной вкладки
        TweenService:Create(
            ts.Tabs[index].Button, 
            TAB_TWEEN, 
            {BackgroundTransparency = 0.4, BackgroundColor3 = ts.Theme.ElementBackground}
        ):Play()
        
        -- Делаем текст и иконку видимыми
        for _, child in pairs(ts.Tabs[index].Button:GetChildren()) do
            if child:IsA("TextLabel") or child:IsA("ImageLabel") then
                TweenService:Create(
                    child, 
                    TAB_TWEEN, 
    ...
    -- Применение темы
    self.TabSystem.ApplyTheme = function(ts, theme)
        ts.Theme = theme
        
        -- Обновляем все вкладки
        for _, tab in pairs(ts.Tabs) do
            local isSelected = (ts.SelectedTab == tab.Index)
